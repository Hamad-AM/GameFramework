...
vec3 AGXTonemapper(vec3 color) {
    // AgX constants
    const float AGX_LOOK_CONTRAST = 1.6;
    const float AGX_LOOK_SATURATION = 1.2;
    const float exposure = 2;
    
    // Pre-tonemapper gamut mapping matrix (convert to AgX working space)
    mat3 agxMatIn = mat3(
        0.84247906, 0.0784336, 0.07922375,
        0.04232824, 0.87846864, 0.07914338,
        0.04233838, 0.0784336, 0.87914705
    );
    
    // Post-tonemapper gamut mapping matrix (convert back)
    mat3 agxMatOut = mat3(
        1.19687900, -0.0528968, -0.14400800,
        -0.04277061, 1.1387153, -0.09590542,
        -0.04277061, -0.0528968, 1.1056705
    );
    
    // Pre-exposure adjustment (optional, adjust exposure)
    color *= pow(2.0, -exposure);
    
    // Main AgX tonemapping
    
    // Convert to working space
    vec3 agxColor = color * agxMatIn;
    
    // Log encoding
    vec3 logColor;
    logColor.r = log2(max(agxColor.r, 1e-5));
    logColor.g = log2(max(agxColor.g, 1e-5));
    logColor.b = log2(max(agxColor.b, 1e-5));
    
    // Apply contrast
    logColor = AGX_LOOK_CONTRAST * (logColor + 10.0) - 10.0;
    
    // Convert back to linear
    agxColor.r = pow(2.0, logColor.r);
    agxColor.g = pow(2.0, logColor.g);
    agxColor.b = pow(2.0, logColor.b);
    
    // Soft clip
    vec3 one = vec3(1.0);
    agxColor = one - exp(-agxColor);
    
    // Apply saturation
    float luma = dot(agxColor, vec3(0.2126, 0.7152, 0.0722));
    agxColor = mix(vec3(luma), agxColor, AGX_LOOK_SATURATION);
    
    // Convert back to output space
    return agxColor * agxMatOut;
}
