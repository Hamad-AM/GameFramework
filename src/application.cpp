...
namespace atl
{
    ...
    void application::initialize(u32 screen_width, u32 screen_height)
    {
        ...
        sponza = load_model("assets/untitled.glb");
    ...
    // -----------------------------------------------------
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, screen_width_, screen_height_, 0, GL_RGBA, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        ...
        u32 hdrTexture = SkyBox("assets/hdri/golden_savanna_horizon_4k.hdr");
        ...
        equirectangularToCubemapShader.uniform_int("equirectangularMap", 0);
        equirectangularToCubemapShader.uniform_matrix4("projection", captureProjection);
        ...
        for (unsigned int i = 0; i < 6; ++i)
        {
            equirectangularToCubemapShader.uniform_matrix4("view", captureViews[i]);
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        // cdFramebuffer(GL_FRAMEBUFFER, 0);

        lightPosition = glm::vec3(0.5f, 2.5f, 0.5f);
        ...
        while (!window->should_close())
        {
            ...
            glBindTexture(GL_TEXTURE_2D, positionBuffer);
            SSAOShader.uniform_int("positionBuffer", 2);
            glActiveTexture(GL_TEXTURE3);
            glBindTexture(GL_TEXTURE_2D, noiseTexture);
            SSAOShader.uniform_int("texNoise", 3);
            GLuint program = SSAOShader.program();
        ...
        //
            ...
            backgroundShader.uniform_matrix4("view", camera.view);
            backgroundShader.uniform_matrix4("projection", camera.projection);
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
            renderCube();
            //glDepthFunc(GL_LESS);
    ...
    void application::update(f32 dt)
    {
        ...
        shader.uniform_vector3f("lightPosition", lightPosition);
        shader.uniform_vector3f("cameraPosition", camera.position);
        ...
        for (u32 i = 0; i < meshRenderData.size(); ++i)
        {
            ...
            shader.uniform_int("metallicRoughness", 4);
    ...
    void application::BatchModel(Model& model)
    {
        ...
        for (const Mesh& mesh : model.meshes)
        {
            ...
            {
                std::shared_ptr<Image>& image = images[mesh.metallicRoughness];
                ...
                renderData.metallicRoughnessID = textureID;
            }
            else {
                renderData.metallicRoughnessID = gpu_textures[mesh.metallicRoughness];
            }
    ...
    u32 application::SkyBox(const char* filePath)
    {
