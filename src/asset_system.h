...
Model GltfToModel(tinygltf::Model& model, std::unordered_map<s32, std::shared_ptr<Image>>& images)
{
    Model result;
               ...
               std::unordered_map<s32, u32> gpu_textures)
{
    ...
    if (succesful)
    {
        model = GltfToModel(glModel, images);
    }
    BatchModel(model, batchMeshRenderData, images, gpu_textures);
}
